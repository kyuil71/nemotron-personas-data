...
const SCAN_DEFAULT = 30000; // 부분검색 시 기본 최대 검색 행 수
const SCAN_MAX = 200000;    // 브라우저 안정성을 위한 부분검색 상한
...
// Hugging Face Dataset Viewer /filter는 LIKE를 안정적으로 지원하지 않는다.
// 따라서 서버에는 공식 지원되는 정확 일치/범위 조건만 보내고,
// 직업·도시·지역 같은 부분검색은 가져온 행을 브라우저에서 후처리한다.
...
function buildQueryPlan() {
  return {
    serverWhere: buildServerWhere(),
    clientFilters: getClientContainsFilters(),
  };
}
...
function classifyHttp(status, body) {
  ...
  if (status === 429) return "retry";                       // 속도 제한
  if (status === 404) return "notfound";                    // 데이터셋 없음
  if (status === 401 || status === 403) return "auth";      // 권한/게이트
  if (b.includes("loading") || b.includes("index") || b.includes("try again"))
    return "warming";                                        // 색인 준비 중
  if (status >= 500) return "retry";                        // 서버 일시 오류
  return "fatal";
}

// 준비될 때까지 폴링하는 견고한 fetch. "색인 로딩(500)"을 정상 대기 상태로 취급한다.
async function fetchJson(url, opts = {}) {
  ...
  while (true) {
    ...
    if (kind === "notfound")
      throw new Error("데이터셋을 찾을 수 없습니다 (404). 저장소 이름을 확인하세요.");
    if (kind === "auth")
      throw new Error("접근 권한이 필요합니다. 고급 설정에서 HF 토큰을 입력해 보세요 (401/403).");
    if (kind === "fatal")
      throw new Error(`HTTP ${status} ${(body || "").slice(0, 160)}`);

    const elapsed = Date.now() - start;
    if (elapsed > maxWaitMs) {
      if (kind === "warming")
        throw new Error("데이터 색인 준비가 예상보다 오래 걸립니다. 1~2분 뒤 다시 시도해 주세요.");
      throw new Error(netErr ? ("네트워크 오류: " + netErr.message)
        : `HTTP ${status} ${(body || "").slice(0, 160)}`);
    }
    if (onWait) onWait({ warming: kind === "warming", sec: Math.round(elapsed / 1000), attempt });
    const backoff = kind === "warming"
      ? Math.min(2500 + attempt * 1000, 6000)
      : Math.min(700 * attempt, 4000);
    await sleep(backoff);
  }
}
...
function containsPlaceholder(col) {
  const country = COUNTRIES.find((x) => x.dataset === state.dataset);
  const isKorea = country && country.name === "한국";
  if (col === "occupation")
    return isKorea ? "예: 교사 / 의사 / 개발" : "예: teacher / engineer / designer";
  if (["city", "district", "state", "province", "region", "prefecture"].includes(col))
    return isKorea ? "예: 서울 / 강남 / 경기" : "예: New York / Tokyo / Paris";
  return isKorea ? "일부 단어 입력" : "영어/현지어 일부 단어 입력";
}
...
async function collect() {
  ...
  const scanInput = document.querySelector("#maxScan");
  const rawScan = scanInput ? parseInt(scanInput.value, 10) : SCAN_DEFAULT;
  const maxScan = Math.min(Math.max(rawScan || SCAN_DEFAULT, maxN), SCAN_MAX);
  ...
  // 부분검색이 없는 경우: 기존처럼 서버 필터 결과를 바로 수집한다.
  if (!hasClientFilters) {
    let offset = 0;
  ...
  // 부분검색이 있는 경우: LIKE를 서버에 보내지 않고, 서버 지원 조건으로만 행을 가져온 뒤
  // 브라우저에서 occupation/city/district 등의 포함 조건을 검사한다.
  const rows = [];
  const scanLimit = total != null ? Math.min(maxScan, total) : maxScan;
  let scanned = 0;
  let offset = 0;
  if (mode === "random" && total && total > PAGE)
    offset = Math.floor(Math.random() * Math.max(1, total - PAGE));

  while (rows.length < maxN && scanned < scanLimit) {
    let need = Math.min(PAGE, scanLimit - scanned);
    if (total != null) {
      if (offset >= total) offset = 0;
      need = Math.min(need, total - offset);
      if (need <= 0) { offset = 0; continue; }
    }
  ...
  const finalRows = rows.slice(0, maxN);
  if (!finalRows.length) {
    setProgress(`완료 — ${scanned.toLocaleString()}행을 확인했지만 조건에 맞는 사람이 없습니다. 해외 데이터는 영어/현지어 표기로 입력해야 합니다.`);
    return [];
  }
