 ...
 * - 한국 데이터셋만 사용한다.
 * - 1차: 정확 조건은 서버 /filter, 키워드는 브라우저 OR 판정으로 최대 5,000명 후보 수집.
...
function renderExactFilters(fields) {
  ...
  root.innerHTML = "";
  if (!fields.length) {
    root.appendChild(el("p", { class: "fine" }, "표시할 정확 조건이 없습니다. 1차 키워드 또는 무조건 수집으로 진행할 수 있습니다."));
    return;
  }
...
function getMaxScan(maxN) {
  ...
  return Math.min(Math.max(raw || 100000, maxN, SCAN_MIN), SCAN_MAX);
}
...
async function collectStage1() {
  ...
  const keywords = parseKeywords($("#stage1Keywords").value);
  const keywordCols = getSelectedColumns("#stage1Columns");
  ...
  // 키워드가 있으면 정확 조건으로 줄인 후 브라우저에서 OR 판정한다.
  const scanLimit = total != null ? Math.min(maxScan, total) : maxScan;
  const getNextOffset = nextOffsetFactory(total, mode);
  const rows = [];
  let scheduled = 0;
  let scanned = 0;
  const shouldStop = () => state.cancelRequested || rows.length >= maxN || scheduled >= scanLimit;
  ...
  async function scanWorker() {
    while (!shouldStop()) {
      ...
      const offset = getNextOffset();
      if (offset == null) break;
      let need = Math.min(PAGE, scanLimit - scheduled);
      if (total != null) need = Math.min(need, Math.max(0, total - offset));
      if (need <= 0) break;
      scheduled += need;
      const data = await fetchJson(buildUrl(where, offset, need), { onWait });
      ...
      scanned += got.length;
      for (const r of got) {
        if (matchesKeywords(r, keywordCols, keywords, "any")) {
          rows.push(r);
          if (rows.length >= maxN) break;
        }
      }
      setProgress(`1차 OR 검색 중… ${formatN(scanned)}행 확인 · 후보 ${formatN(rows.length)} / ${formatN(maxN)}명 · 키워드 ${formatN(keywords.length)}개` + (where ? ` · ${scopeTxt} ${totalTxt}명` : "") + (mode === "random" ? " · 무작위" : "") + (partial ? " · 부분 인덱스" : ""));
      if (got.length === 0) break;
    }
  }
  await Promise.all(Array.from({ length: scanConcurrency() }, scanWorker));
  if (!rows.length) setProgress(`${formatN(scanned)}행을 확인했지만 1차 키워드에 맞는 후보가 없습니다. 키워드를 넓히거나 최대 검색 행 수를 늘려 보세요.`);
  return rows.slice(0, maxN);
}
...
function resetAll() {
  resetExactFilters();
  $("#stage1Keywords").value = "";
  $("#maxN").value = String(HARD_CAP);
  $("#maxScan").value = "100000";
  $("#mode").value = "head";
  setColumnSelection("#stage1Columns", "text");
  setColumnSelection("#stage2Columns", "text");
  state.stage1Rows = [];
  state.finalRows = [];
  state.previewRows = [];
...
async function runStage1() {
  ...
  try {
    ...
    if (rows.length) {
      ...
      setBanner("1차 후보가 수집되지 않았습니다. 정확 조건을 완화하거나 키워드를 넓혀 보세요.", "");
    }
  } catch (e) {
    console.error(e);
...
async function init() {
  ...
  renderExactFilters(state.exactFields);
  renderColumnChecks("#stage1Columns", defaultKeywordCols());
  ...
  $("#stage1ColsText").onclick = () => setColumnSelection("#stage1Columns", "text");
  $("#stage1ColsAll").onclick = () => setColumnSelection("#stage1Columns", "all");
