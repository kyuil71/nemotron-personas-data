...
// 키워드로 요약하기 어려운 장문 서술형(프로즈) 컬럼 → 필터에서 제외(엑셀에는 포함)
const PROSE_EXCLUDE_RE = /(^persona$|_persona$|persona_|description|narrative|biography|^bio$|_bio$|(^|_)summary($|_)|profile|about_me)/i;
// 목록 중복 컬럼(_list) → 필터에서 제외(간결)
...
// 통계 응답의 컬럼 타입(int/float/string_text/string_label/class_label/list/bool 등)
...
// 숫자 컬럼 여부. string_text/list의 min/max는 '글자 수·길이' 통계이므로 값으로 오인하지 않는다.
function isNumericColumn(colType, col, minMax) {
  ...
  if (NUMERIC_TYPE_RE.test(t)) return true;         // 실제 숫자 타입만 범위로
  if (STRINGISH_TYPE_RE.test(t)) return false;      // 문자열/목록/불리언 등은 값 아님(길이 통계)
  // 타입 불명확: 이름이 명백히 수치형일 때만 허용
  return col === "age" || /(^|_)(age|year|income|salary|amount|score|count|number)(_|$)/i.test(col);
}
function classifyField(col, cs, sourceIndex, colType) {
  if (!col || INTERNAL_COL_RE.test(col)) return null;
  if (LIST_DUP_RE.test(col)) return null;           // _list 중복 컬럼 제외
  if (PROSE_EXCLUDE_RE.test(col)) return null;       // 장문 서술형 프로즈 제외(키워드화 어려움)
  ...
  // string_text 백스톱: 이름 규칙을 피한 초장문 프로즈는 키워드 검색도 무의미 → 제외
  if (/string_text/.test(String(colType))) {
    const typicalLen = Number(cs.mean ?? cs.median ?? 0);
    if (typicalLen > 1000 || Number(cs.max ?? 0) > 5000) return null;
  }

  // 키워드 포함검색. 상위 값이 있으면 빠른 선택 칩으로 병행 제공.
  const topValues = freq
    ? Object.entries(freq)
        ...
        .slice(0, 12)
    : [];
  return { col, control: "contains", sourceIndex, uniqueCount, topValues, keyword: !topValues.length };
}
function buildFields(stats) {
  ...
  (stats || []).forEach((s, idx) => {
    ...
    const cs = getColumnStats(s);
    const f = classifyField(col, cs, idx, getColumnType(s));
    if (f) fields.push(f);
  });
  fields.sort((a, b) => {
...
function containsPlaceholder(col) {
  ...
  // 서술형(문화적 배경·경력 목표·취미·기술 등) 키워드 예시
  if (/hobbies|interests|skills|expertise|goals|ambitions|cultural|culture|background|traits|personality/i.test(col)) {
    if (c.name === "한국") return "키워드 예: 창업 / 여행 / 리더십";
    return "키워드 예: startup / travel / leadership";
  }
  return c.name === "한국" ? "키워드 입력" : "키워드 입력 (현지어/영어)";
}
