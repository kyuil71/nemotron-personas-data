...
function renderFilters(optionMap) {
  ...
  for (const f of FIELDS) {
    ...
    if (f.control === "range") {
      ...
      // "상관 없음" 토글 (기본 켜짐 = 나이 조건 없음, 입력칸 비활성)
      ...
      const anyRow = el("label", { class: "opt any" },
        [anyCb, document.createTextNode(ANY_LABEL)]);

      const box = el("div", { class: "rangebox" }, [anyRow, el("div", { class: "rangebody" }, row)]);
      wrap.appendChild(box);
      wrap.appendChild(el("div", { class: "hint" }, "나이(만) 범위 · 세 단위"));
      applyAny();
    }
...
function buildMultiSelect(col, options) {
  ...
  // 맨 위 "상관 없음" (선택 값이 없으면 자동으로 켜짐, 특정 값과 상호배타)
  const anyCb = el("input", { type: "checkbox" });
  ...
  function syncAny() { anyCb.checked = isAny(); }

  anyCb.onchange = () => {
    if (anyCb.checked) {                      // 상관 없음 -> 특정 선택 모두 해제
      state.category[col] = [];
      optionCbs.forEach((c) => (c.checked = false));
    } else {
      anyCb.checked = true;                   // 스스로는 끌 수 없음 (특정 값 선택으로만 해제됨)
    }
  };
